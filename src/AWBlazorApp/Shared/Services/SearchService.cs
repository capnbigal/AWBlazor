...
/// <param name="Url">Deep link — the record's list page pre-filtered to the search term.</param>
...
public sealed class SearchService(LookupService lookup)
{
    ...
    /// entity's list page with a <c>?q=</c> param so the destination grid pre-filters to the term.
    ...
    public async Task<IReadOnlyList<RecordHit>> SearchRecordsAsync(string? query, int perCategory = 4)
    {
        ...
        var qp = Uri.EscapeDataString(query);
        var hits = new List<RecordHit>();

        void Add(Task<IEnumerable<LookupItem<int>>> task, string category, string icon, string listRoute)
        {
            foreach (var item in task.Result.Take(perCategory))
                hits.Add(new RecordHit(category, item.DisplayText, icon, $"{listRoute}?q={qp}"));
        }
