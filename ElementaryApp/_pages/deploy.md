...
```bash
x new blazor-vue ElementaryApp
```

2. Generate deployment SSH key:
```bash
ssh-keygen -t ed25519 -C "deploy@elementaryapp" -f ./deploy-key
```
...
```yaml
service: elementaryapp
image: your-github-username/elementaryapp
...
proxy:
  ssl: true
  host: elementaryapp.example.com
```
