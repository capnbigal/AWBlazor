...
Since EF Core migrations never touch this table, the test environment needs special handling. The integration test fixture in `ElementaryApp.Tests` runs an idempotent `CREATE TABLE IF NOT EXISTS` SQL block during `OneTimeSetUp` to ensure the `ToolSlotConfigurations` table exists in the `AdventureWorks2022_dev` database. Tests that write rows to this table are responsible for cleaning up after themselves to avoid polluting the shared dev database.
