...
public sealed class SavedQueryRunner(IDbContextFactory<ApplicationDbContext> dbFactory)
{
    ...
    private static async Task<double> ScalarAsync(ApplicationDbContext db, SavedQuery q, CancellationToken ct)
    {
        ...
        {
            QueryMetric.SalesOrderCount        => await db.SalesOrderHeaders.Where(DateFilter<Features.Sales.SalesOrderHeaders.Domain.SalesOrderHeader>(q, o => o.OrderDate)).CountAsync(ct),
            QueryMetric.TotalSalesRevenue      => (double)await db.SalesOrderHeaders.Where(DateFilter<Features.Sales.SalesOrderHeaders.Domain.SalesOrderHeader>(q, o => o.OrderDate)).SumAsync(o => o.TotalDue, ct),
            QueryMetric.AverageOrderValue      => (double)await db.SalesOrderHeaders.Where(DateFilter<Features.Sales.SalesOrderHeaders.Domain.SalesOrderHeader>(q, o => o.OrderDate)).AverageAsync(o => o.TotalDue, ct),
            QueryMetric.OpenWorkOrderCount     => await db.WorkOrders.CountAsync(w => w.EndDate == null, ct),
            QueryMetric.WorkOrderCount         => await db.WorkOrders.Where(DateFilter<Features.Production.WorkOrders.Domain.WorkOrder>(q, w => w.StartDate)).CountAsync(ct),
            QueryMetric.PurchaseOrderCount     => await db.PurchaseOrderHeaders.Where(DateFilter<Features.Purchasing.PurchaseOrderHeaders.Domain.PurchaseOrderHeader>(q, p => p.OrderDate)).CountAsync(ct),
            QueryMetric.TotalPurchaseSpend     => (double)await db.PurchaseOrderHeaders.Where(DateFilter<Features.Purchasing.PurchaseOrderHeaders.Domain.PurchaseOrderHeader>(q, p => p.OrderDate)).SumAsync(p => p.TotalDue, ct),
    ...
    private static async Task<List<QueryBucket>> GroupedAsync(ApplicationDbContext db, SavedQuery q, CancellationToken ct)
    {
        ...
        return q.Metric switch
        {
            QueryMetric.SalesOrderCount => ToBuckets(await db.SalesOrderHeaders
                .Where(DateFilter<Features.Sales.SalesOrderHeaders.Domain.SalesOrderHeader>(q, o => o.OrderDate))
                .GroupBy(IntBucketSelector<Features.Sales.SalesOrderHeaders.Domain.SalesOrderHeader>(o => o.OrderDate, g))
            ...
            QueryMetric.WorkOrderCount => ToBuckets(await db.WorkOrders
                .Where(DateFilter<Features.Production.WorkOrders.Domain.WorkOrder>(q, w => w.StartDate))
                .GroupBy(IntBucketSelector<Features.Production.WorkOrders.Domain.WorkOrder>(w => w.StartDate, g))
                .Select(grp => new BucketRow(grp.Key, grp.Count()))
                .ToListAsync(ct), g),

            QueryMetric.PurchaseOrderCount => ToBuckets(await db.PurchaseOrderHeaders
                .Where(DateFilter<Features.Purchasing.PurchaseOrderHeaders.Domain.PurchaseOrderHeader>(q, p => p.OrderDate))
                .GroupBy(IntBucketSelector<Features.Purchasing.PurchaseOrderHeaders.Domain.PurchaseOrderHeader>(p => p.OrderDate, g))
                .Select(grp => new BucketRow(grp.Key, grp.Count()))
                .ToListAsync(ct), g),
